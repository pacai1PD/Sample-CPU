...
1. 该CPU现在可以执行`ori lui addiu beq`这四种指令，但并没有完成数据相关，请自行添加相关数据通路。例如添加MEM/EX段的回传到ID段的连线。该部分实际对应的是五级流水中出现的数据冒险问题，即第一条指令最后的寄存器写入结果在紧随其后的指令中需要被使用，但是由于这条指令还没有运行到写入阶段，所以需要添加对应连线将该指令的数值传递到前几个阶段正在运行的指令中去，连线添加形式参考源代码中ID.v的id_to_ex_bus。同时在添加连线和定义其他线路参数时，除了在对应的ID或其他段定义外，大家也需要根据实际情况在mycpu_core和mycpu_top中添加对应参数定义。
